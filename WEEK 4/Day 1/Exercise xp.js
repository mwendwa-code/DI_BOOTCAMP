...
// Exercise 2: Colors #2
{
	...
	colors.forEach((color, index) => {
		...
		const suffix = position > 3 ? ordinal[0] : ordinal[position];
...
// Exercise 4: Employees
{
	...
	const fullStackResidentLastNames = fullStackResidents.map(
		(user) => user.lastName
	);
...
// Exercise 5: Star Wars
{
	...
	const sentence = epic.reduce((result, word) => `${result} ${word}`);
...
{
	const students = [
		{ name: "Alice", course: "Computer Science", isPassed: true },
		{ name: "Ian", course: "Computer Science", isPassed: false },
